...
def _beam_search_one_step(_step_score, _state, output_score, number_of_samples, beam_size, state_dim, output_score_list, prev_output_index_list, output_label_id_list, embedding, _tensors_to_debug=None):
    number_of_samples = K.shape(_step_score)[0] / beam_size
...
def get_k_best_from_lattice(lattice, k=1, eos=None):
    ...
    path_score, output_indice = top_k (output_score_list[0], k)    # shape: nb_samples,k
    # fill path and update path_score
    path = []
    ...
    path = K.permute_dimensions(K.pack(path), (1, 2, 0))    # time_steps, nb_samples, k -> nb_samples, k, time_steps
...
if K._BACKEND == 'theano':
    ...
    def gather_by_sample(x, indices):
        ...
        return tf.scan(_step , elems, initializer=tf.constant(0, dtype='int32'))
